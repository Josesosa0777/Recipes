...
class ChefsEditTest < ActionDispatch::IntegrationTest
	...
	test "reject an invalid edit" do
 	...
 	test "accept edit attempt by admin user" do
	    ...
	    get edit_chef_path(@chef)
	    assert_template 'chefs/edit'
	    patch chef_path(@chef), params: { chef: { chefname: "jose3", 
	                        email: "jose3@example.com" } }
	    assert_redirected_to @chef
	    assert_not flash.empty?
	    @chef.reload
	    assert_match "jose3", @chef.chefname
	    assert_match "jose3@example.com", @chef.email
	end
  	...
  	test "redirect edit attempt by another non-admin user" do
	    ...
	    assert_not flash.empty?
	    @chef.reload
	    assert_match "jose", @chef.chefname
	    assert_match "jose@example.com", @chef.email
  	end
  # test "the truth" do
  #   assert true
  # end
